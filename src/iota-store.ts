...
export class IotaJwkStore implements JwkStorage {
  ...
  public static async build(storage: StorageSpec<any, any>, alias: string) {
    ...
    if (
      ...
      jwkStore._keys = jwkStore.deserializeKeyMap(keyMapExists.extras.keysMap);
    }
    return jwkStore;
  }
  ...
  public async getPrivateKeyFromStore(keyId: string) {
    const jwk = this._keys.get(keyId);
    const [privateKey, _] = await decodeJwk(jwk);
    return privateKey;
  }
